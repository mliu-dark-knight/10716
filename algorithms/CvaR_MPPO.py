...
class QRVNetwork(object):
    def __init__(self, hidden_dims, scope, value_reg, n_agent, n_quantile):
        self.hidden_dims = hidden_dims
        self.scope = scope
        ...
        with tf.variable_scope(self.scope, reuse=tf.AUTO_REUSE):
            ...
            for agent in range(self.n_agent):
                quantile = tf.layers.dense(hidden, self.hidden_dims[1], activation=tf.nn.tanh,
                                             ...
                                             kernel_initializer=tf.initializers.orthogonal(),
                                             use_bias=False)
                quantile = tf.layers.dense(quantile, self.n_quantile, activation=None,
                                         kernel_regularizer=tf.contrib.layers.l2_regularizer(self.value_reg),
                                         bias_regularizer=tf.contrib.layers.l2_regularizer(self.value_reg),
                                         kernel_initializer=tf.initializers.orthogonal(),
                                         use_bias=False)[:,:, None]
...
class CvaRMPPO(object):
    ...
    def build_critic(self):
        self.critic = QRVNetwork(self.hidden_dims, 'critic', self.value_reg, self.n_agent, self.n_quantile)
    ...
    def build_critic_loss(self):
        ...
        quantiles_id = tf.concat([tf.range(batch_size, dtype=tf.int32)[:, None], self.agent_id[:, None]], axis=1)
        Z = tf.gather_nd(self.critic(self.states), quantiles_id)
        self.values = tf.reduce_mean(Z, axis=1)[:, None]
        errors = self.returns[:, None] - Z
        ...
        quantiles = tf.expand_dims(tf.linspace(1. / self.n_quantile, 0.05-1. / self.n_quantile, self.n_quantile), axis=0)
        quantile_huber_loss = (tf.abs(quantiles - tf.stop_gradient(tf.to_float(errors < 0))) * huber_loss) / self.kappa
