...
class RunningStatExp(object):
    ...
    def push(self, x):
        ...
        if self._n == 1:
            ...
            oldM = self._M.copy()
            self._M[...] = (1-self.lr)*oldM + self.lr*x
            self._S[...] = (1-self.lr)*self._S + self.lr*(x-oldM)*(x-self._M)
    ...
    @property
    def var(self):
        return self._S if self._n > 1 else np.square(self._M)
...
class ZFilter:
    ...
    def __init__(self, shape, demean=True, destd=True, clip=10.0, lr=None):
        self.demean = demean
        self.destd = destd
        self.clip = clip
        ...
        if not lr is None:
            self.rs = RunningStatExp(shape, lr)
