...
def parse_arguments():
	...
	parser.add_argument('--tau', default=1e-3, type=float, help='algorithms soft parameter update tau')
	...
	parser.add_argument('--epsilon', default=0.1, type=float, help='Exploration noise, fixed in D4PG')
	parser.add_argument('--train-episodes', default=100000, type=int, help='Number of episodes to train')
	parser.add_argument('--save-episodes', default=10000, type=int, help='Number of episodes to save model')
		...
		if not args.eval:
			if args.model == 'DDPG':
				total_rewards = agent.train(
					sess, saver, model_path, batch_size=args.batch_size, step=args.step,
